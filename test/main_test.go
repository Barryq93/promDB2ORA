...
`
    ...
    // Mock DB client creation
    originalNewDBClient := db.NewDBClient
    defer func() { db.NewDBClient = originalNewDBClient }()
    db.NewDBClient = func(conn app.Connection) (*db.DBClient, error) {
...
func TestExecuteQuery(t *testing.T) {
    ...
    job := app.QueryJob{
        Query: app.Query{
            ...
            Query:   "SELECT 1",
            Timeout: 1,
            Gauges:  []app.Gauge{{Name: "test_gauge", Desc: "Test gauge", Col: 1}},
        },
        Connection: app.Connection{DBName: "testdb", DBType: "DB2", ExtraLabels: map[string]string{"dbinstance": "test"}},
        Context:    context.Background(),
    }
    appInstance.ExecuteQuery(job)
    mockClient.AssertExpectations(t)

    // Failure case with DLQ
    appInstance.ExecuteQuery(job)
...
func TestDeadLetterQueue(t *testing.T) {
    ...
    job := app.QueryJob{
        Query:      app.Query{Name: "TestQuery"},
        Connection: app.Connection{DBName: "testdb"},
        Context:    context.Background(),
    }
    dlq.Add(job)
...
func TestHealthHandler(t *testing.T) {
    ...
    appInstance.workerPool <- app.QueryJob{} // Add a job to test queue length
    ...
    assert.Equal(t, 0, status.CircuitBreakers["testdb"]) // 0 = closed
}

func TestEncryptDecrypt(t *testing.T) {
    key := "32-byte-long-secret-key-here!!" // 32 bytes for AES-256
